...
const FRECENCY_PATH = path.join(Global.Path.state, "frecency.jsonl")

export const { use: useFrecency, provider: FrecencyProvider } = createSimpleContext({
  name: "Frecency",
  init: () => {
    const frecencyFile = file(FRECENCY_PATH)
    onMount(async () => {
      ...
      if (sorted.length > 0) {
        ...
        write(FRECENCY_PATH, content).catch(() => {})
      }
    })
    ...
    function updateFrecency(filePath: string) {
      ...
      appendFile(FRECENCY_PATH, JSON.stringify({ path: absolutePath, ...newEntry }) + "\n").catch(() => {})
