...
import { file, which, spawn as compatSpawn, readableStreamToText } from "@/compat"
...
export const rlang: Info = {
  ...
  async enabled() {
    ...
    try {
      const proc = compatSpawn(["air", "--help"], {
        stdout: "pipe",
        stderr: "pipe",
      })
      await proc.exited
...
export const uvformat: Info = {
  ...
  async enabled() {
    ...
    if (which("uv") !== null) {
      const proc = compatSpawn(["uv", "format", "--help"], { stderr: "pipe", stdout: "pipe" })
