...
async function resolveModule(modulePath: string, cwd: string): Promise<string | undefined> {
  try {
    // Try require.resolve with the cwd as a hint
    return require.resolve(modulePath, { paths: [cwd] })
  } catch {
    return undefined
  }
}
...
export namespace LSPServer {
  ...
  export const ESLint: Info = {
    ...
    async spawn(root) {
      const eslint = await resolveModule("eslint", Instance.directory).catch(() => {})
      if (!eslint) return
      ...
      if (!(await file(serverPath).exists())) {
        ...
        await file(zipPath).write(response)
  ...
  export const Oxlint: Info = {
    ...
    async spawn(root) {
      ...
      if (lintBin) {
        const proc = compatSpawn([lintBin, "--help"], { stdout: "pipe" })
        await proc.exited
  ...
  export const Zls: Info = {
    ...
    async spawn(root) {
      ...
      if (!bin) {
        ...
        await file(tempPath).write(downloadResponse)
