...
import { file, write, Glob } from "../compat"
...
export namespace Config {
  ...
  export async function installDependencies(dir: string) {
    ...
    if (!hasGitIgnore) await write(gitignore, ["node_modules", "package.json", "bun.lock", ".gitignore"].join("\n"))
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        ...
        await write(configFilepath, updated).catch(() => {})
      }
  ...
  export async function update(config: Info) {
    ...
    await write(filepath, JSON.stringify(mergeDeep(existing, config), null, 2))
