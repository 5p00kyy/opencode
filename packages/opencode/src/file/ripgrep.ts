...
import { $, file, write, spawn, readableStreamToText, which } from "../compat"
...
export namespace Ripgrep {
  ...
  const state = lazy(async () => {
    let filepath = which("rg")
    if (filepath) return { filepath }
    filepath = path.join(Global.Path.bin, "rg" + (process.platform === "win32" ? ".exe" : ""))

    const rgFile = file(filepath)
    if (!(await rgFile.exists())) {
      ...
      await write(archivePath, new Uint8Array(buffer))
      ...
      if (config.extension === "zip") {
        ...
        await write(filepath, new Uint8Array(await rgBlob.arrayBuffer()))
